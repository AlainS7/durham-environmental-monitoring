...
--    - Example Grafana query: SELECT ts, native_sensor_id, pm2_5 FROM `durham-weather-466502.sensors_shared.tsi_raw_view` WHERE ts >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 7 DAY)
...
FROM `durham-weather-466502.sensors_shared.tsi_raw_view`;

-- Validation
-- SELECT COUNT(*) rows, MAX(ts) max_ts FROM `durham-weather-466502.sensors_shared.tsi_raw_materialized`;
-- SELECT ts, native_sensor_id, pm2_5 FROM `durham-weather-466502.sensors_shared.tsi_raw_view` 
