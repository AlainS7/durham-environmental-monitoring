...
def _build_sensor_catalog() -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for station in get_wu_stations() or []:
        ...
        if not sensor_id:
            continue
        friendly = (
            station.get("friendly_name") or station.get("name") or str(sensor_id)
        ).strip()
        if not friendly:
...
def _augment_catalog_with_data(
    catalog: dict[tuple[str, str], dict[str, Any]], df: pd.DataFrame, sensor_type: str
):
    if (
        df.empty
        or "native_sensor_id" not in df.columns
    ...
    if source == "all":
        async with (
            ...
            wu_task = (
                ...
                else wu_client.fetch_data(start_str, end_str)
            )
            tsi_task = (
                ...
                if (aggregate or agg_interval != "h")
                else tsi_client.fetch_data(start_str, end_str)
            )
            wu_raw, tsi_raw = await asyncio.gather(
                wu_task, tsi_task, return_exceptions=False
            )
    elif source == "wu":
...
def _clean(
    wu_raw: pd.DataFrame, tsi_raw: pd.DataFrame
) -> tuple[pd.DataFrame, pd.DataFrame]:
    wu_df = (
        clean_and_transform_data(wu_raw, "WU") if not wu_raw.empty else pd.DataFrame()
    )
    tsi_df = (
        clean_and_transform_data(tsi_raw, "TSI")
        if not tsi_raw.empty
        else pd.DataFrame()
    )
    log.info(
        f"Fetched WU={len(wu_raw)} TSI={len(tsi_raw)} raw rows -> cleaned WU={len(wu_df)} TSI={len(tsi_df)}"
    )
    return wu_df, tsi_df
    ...
    if os.getenv("GCS_FAKE_UPLOAD") == "1":
        ...
        class _DummyUploader:
            ...
            def upload_parquet(
                ...
                log.info(
                    f"[FAKE] Would upload {len(df)} rows to gs://{self.bucket_name}/{path}"
                )
...
def _safe_upload(
    uploader: Any, df: pd.DataFrame, src: str, aggregate: bool, agg_interval: str
) -> bool:
    """
    ...
    if not aggregate:
        if src == "TSI":
            ...
            if not schema_valid:
                log.error(
                    "TSI schema validation failed - uploading anyway but data quality may be impacted"
                )
                log.debug(f"TSI schema info: {get_schema_info(df)}")
            if not coverage_valid:
                log.warning(
                    "TSI coverage check failed - some critical fields have low coverage"
                )
        elif src == "WU":
            ...
            if not schema_valid:
                log.error(
                    "WU schema validation failed - uploading anyway but data quality may be impacted"
                )
                log.debug(f"WU schema info: {get_schema_info(df)}")
            if not coverage_valid:
                log.warning(
                    "WU coverage check failed - some critical fields have low coverage"
                )
...
def _sink_data(
    ...
    if sink in ("gcs", "both"):
        ...
        if not bucket:
            ...
            wrote_wu = (
                _safe_upload(uploader, wu_df, "WU", aggregate, agg_interval) or wrote_wu
            )
            wrote_tsi = (
                _safe_upload(uploader, tsi_df, "TSI", aggregate, agg_interval)
                or wrote_tsi
            )
...
def _write_bq_staging(
    wu_df: pd.DataFrame, tsi_df: pd.DataFrame, start_str: str, end_str: str
):
    ...
    # Fetch deployment mapping (ids + fingerprint) from BigQuery, then derive sensor_type via config
    try:
        ...
        """
        base_map_df = client.query(deployment_map_sql).to_dataframe()
    except Exception as e:
        log.error(f"Failed to fetch deployment map from BigQuery: {e}")
        base_map_df = pd.DataFrame()

    if base_map_df.empty:
        log.error("Deployment map empty – cannot build BigQuery staging tables.")
        return

    # Build sensor type using repo config (preferred) with light heuristics fallback
    try:
        wu_ids = {
            str((s.get("stationId") or s.get("id") or "")).strip()
            for s in (get_wu_stations() or [])
            if (s.get("stationId") or s.get("id"))
        }
    except Exception:
        wu_ids = set()
    try:
        tsi_ids = {str(d).strip() for d in (get_tsi_devices() or []) if d}
    except Exception:
        tsi_ids = set()

    def _infer_type(nsid: str, src: str) -> str:
        ns = (nsid or "").strip()
        ...
        if s in ("WU", "TSI"):
            return s
        # Heuristics: WU station IDs are typically uppercase alphanumerics like KNCDURHA656
        # TSI device ids in our project often start with 'd' and include digits.
        if ns and ns[0].isalpha() and ns[:2].upper() == "KN":
            return "WU"
        if ns and ns[0].lower() == "d":
            return "TSI"
        return "UNKNOWN"

    base_map_df["sensor_type"] = base_map_df.apply(
        lambda r: _infer_type(str(r["native_sensor_id"]), str(r.get("source", ""))),
        axis=1,
    )
    deployment_map_df = base_map_df[
        ["native_sensor_id", "sensor_type", "deployment_pk"]
    ]

    def _prepare_long(df: pd.DataFrame, sensor_type: str) -> pd.DataFrame:
        if df.empty:
            return pd.DataFrame(
                columns=["timestamp", "deployment_fk", "metric_name", "value"]
            )
        if df.columns.duplicated().any():
            working = df.loc[:, ~df.columns.duplicated()].copy()
        else:
            working = df.copy()
        type_map = deployment_map_df[
            deployment_map_df.sensor_type.str.upper() == sensor_type
        ]
        if type_map.empty:
            log.warning(
                f"No active deployments for type {sensor_type} – skipping staging long melt"
            )
            return pd.DataFrame(
                columns=["timestamp", "deployment_fk", "metric_name", "value"]
            )
        ...
        candidate_vars = [
            ...
            if c not in id_vars + ["native_sensor_id", "sensor_type"]
        ]
        ...
        long_df = long_df.dropna(subset=["value"]).drop_duplicates(
            subset=["timestamp", "deployment_fk", "metric_name"], keep="last"
        )
        return long_df
    ...
    def _split_and_load(long_df: pd.DataFrame, source_label: str):
        ...
        for d in distinct_dates:
            ...
            if "timestamp" not in day_df.columns:
                log.warning(
                    "Skipping %s staging for %s – no timestamp column", source_label, d
                )
                continue
            ...
            if pd.api.types.is_numeric_dtype(ts_series):
                ...
                if pd.isna(max_abs):
                    ...
                    max_abs > 9_007_199_254_740_992
                ):  # larger than float64 safe range -> assume nanoseconds
                    ts_converted = pd.to_datetime(
                        numeric, utc=True, errors="coerce", unit="ns"
                    )
                elif max_abs > 9_007_199_254_740:  # microseconds range
                    ts_converted = pd.to_datetime(
                        numeric, utc=True, errors="coerce", unit="us"
                    )
                else:
                    ts_converted = pd.to_datetime(
                        numeric, utc=True, errors="coerce", unit="s"
                    )
            else:
                ts_converted = pd.to_datetime(ts_series, utc=True, errors="coerce")
            ts_converted = (
                ts_converted.dt.tz_convert("UTC")
                if ts_converted.dt.tz is not None
                else ts_converted.dt.tz_localize("UTC")
            )
            ts_py = pd.Series(
                [
            ...
            def _ensure_py_datetime(value: Any) -> Optional[datetime]:
                ...
                if isinstance(value, (np.integer, int)):
                    ...
                    if magnitude > 9_007_199_254_740_992:
                        dt = pd.to_datetime(
                            int(value), utc=True, errors="coerce", unit="ns"
                        )
                    elif magnitude > 9_007_199_254_740:
                        dt = pd.to_datetime(
                            int(value), utc=True, errors="coerce", unit="us"
                        )
                    else:
                        dt = pd.to_datetime(
                            int(value), utc=True, errors="coerce", unit="s"
                        )
                    if pd.isna(dt):
                        return None
                    # Return timezone-naive datetime
            ...
            if day_df.empty:
                log.warning(
                    "No valid timestamps remain for %s on %s after coercion",
                    source_label,
                    d,
                )
                continue
            ...
            numeric_mask = day_df["timestamp"].apply(
                lambda v: isinstance(v, (np.integer, int, np.floating, float))
            )
            if numeric_mask.any():
                ...
                day_df.loc[numeric_mask, "timestamp"] = day_df.loc[
                    numeric_mask, "timestamp"
                ].apply(_ensure_py_datetime)
            non_dt_mask = day_df["timestamp"].apply(
                lambda v: v is not None and not isinstance(v, datetime)
            )
            if non_dt_mask.any():
                ...
                day_df.loc[non_dt_mask, "timestamp"] = day_df.loc[
                    non_dt_mask, "timestamp"
                ].apply(_ensure_py_datetime)
            ...
            if invalid_final.any():
                ...
                if day_df.empty:
                    log.warning(
                        "No valid timestamps remain for %s on %s after final coercion",
                        source_label,
                        d,
                    )
                    continue
            type_counts = (
                ensured_final.apply(lambda v: type(v).__name__).value_counts().to_dict()
            )
            log.warning(
            ...
            try:
                ...
                # If the project is in BigQuery Sandbox (e.g., immediately after billing changes),
                # ensure the table has an expiration < 60 days to satisfy sandbox constraints.
                try:
                    tbl.expires = datetime.utcnow() + timedelta(days=30)
                except Exception:
                    pass
            ...
            load_job = client.load_table_from_dataframe(
                day_df[["timestamp", "deployment_fk", "metric_name", "value"]],
                fq,
    ...
    for i in range(total_days):
        ...
        try:
            ...
            if config.is_dry_run:
                ...
                continue
            wrote_wu, wrote_tsi = _sink_data(
                wu_df, tsi_df, config.sink, config.aggregate, config.agg_interval
            )
            try:
                _write_bq_staging(wu_df, tsi_df, day_str, day_str)
            except Exception:
                log.error(
                    f"Unhandled error while writing BigQuery staging tables for {day_str}",
                    exc_info=True,
                )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="How many days back from today (start inclusive)",
    )
    p.add_argument(
        "--start", type=str, help="Explicit start date YYYY-MM-DD (overrides --days)"
    )
    p.add_argument(
        "--end", type=str, help="Explicit end date YYYY-MM-DD (default today)"
    )
    ...
    if args.start:
        ...
        if ingest_env and args.days == 0:  # environment-provided explicit date
            try:
                ...
                log.warning(
                    f"Invalid INGEST_DATE '{ingest_env}' – falling back to days offset"
                )
                end = datetime.utcnow()
