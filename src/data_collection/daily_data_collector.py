...
def _write_bq_staging(
    ...
    try:
        ...
        SELECT
            native_sensor_id,
            -- Derive sensor type from sensor_id prefix:
            --   MS-* → WU (Weather Underground stations)
            --   AA-* / BS-* → TSI (indoor/outdoor air quality sensors)
            CASE
                WHEN STARTS_WITH(sensor_id, 'MS-') THEN 'WU'
                ELSE 'TSI'
            END AS sensor_type,
